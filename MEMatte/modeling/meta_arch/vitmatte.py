...
class ViTMatte(nn.Module):
    ...
    def forward(self, batched_inputs, patch_decoder=False):
        ...
        if self.training:
            ...
            if self.balance_loss == "Direct":
                total_ratio = sum([p.sum() / p.numel() for p in out_pred_prob]) / len(out_pred_prob)
                losses['direct_ratio_loss'] = total_ratio * self.token_loss_ratio
            elif self.balance_loss == "MSE": # design new loss
                total_ratio = sum([p.sum() / p.numel() for p in out_pred_prob]) / len(out_pred_prob)
                losses['mse_ratio_loss'] = self.token_loss_ratio * F.mse_loss(total_ratio, torch.tensor(self.backbone.topk).cuda())
            elif self.balance_loss == "Each_Router":
                each_ratios = [p.sum() / p.numel() for p in out_pred_prob]
                for i, each_ratio in enumerate(each_ratios):
                    losses[f'mse_ratio_loss_{i}'] = self.token_loss_ratio * F.mse_loss(each_ratio, torch.tensor(self.backbone.topk).cuda())
            elif self.balance_loss == "FLOPs_Loss":
                flops_losses = []
                for i in range(len(out_pred_prob)):
                    for j in range(int(out_pred_prob[i].shape[0])):
                        ratio = out_pred_prob[i][j].sum() / out_pred_prob[i][j].numel()
                        flops_loss = 2 * 384 * ratio + 32 * 32 * ratio * ratio
                        flops_losses.append(flops_loss)
                losses['flops_ratio_loss'] = self.token_loss_ratio * (sum(flops_losses) / len(flops_losses))

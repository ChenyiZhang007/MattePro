...
class Attention(nn.Module):
    ...
    # def forward(self, x, policy, H, W): # total FLOPs: 4 * B * hw * dim * dim +  2 * B * hw * hw * dim
    #     B, N, C = x.shape
    #     # N = H*W if x.ndim == 4 else x.shape[1]
    #     # B, N, C = x.shape
    ...
    #     if self.use_rel_pos:
    #         attn = add_decomposed_rel_pos(attn, q, self.rel_pos_h, self.rel_pos_w, (H, W), (H, W), policy=policy)
    ...
    #     x = (attn @ v).view(B, self.num_heads, N, -1).permute(0, 2, 1, 3).reshape(B, N, -1) # 5.637144576 (B * hw * hw * dim)
    #     # x = (attn @ v).view(B, self.num_heads, H, W, -1).permute(0, 2, 3, 1, 4).reshape(B, H, W, -1)
    #     x = self.proj(x) # 2.113929216 (B * hw * dim * dim) 14 * 1024 * 384 * 384
...
class EfficientBlock_MSA_PRO(nn.Module):
    """Efficient Block to replace the Original Transformer Block"""
    def __init__(self, dim, expand_ratio = 2, kernel_size = 5):
        super(EfficientBlock_MSA_PRO, self).__init__()
...
class EfficientBlock_ECA_wo_DWC(nn.Module):
    """Efficient Block to replace the Original Transformer Block"""
    def __init__(self, dim, expand_ratio = 2, kernel_size = 5):
        super(EfficientBlock_ECA_wo_DWC, self).__init__()
        self.fc1 = nn.Linear(dim, dim*expand_ratio)
        self.act1 = nn.GELU()
        self.fc2 = nn.Linear(dim*expand_ratio, dim)
        self.eca = ECA(dim)
...
class EfficientBlock_ECA_wo_shortcut(nn.Module):
    """Efficient Block to replace the Original Transformer Block"""
    def __init__(self, dim, expand_ratio = 2, kernel_size = 5):
        super(EfficientBlock_ECA_wo_shortcut, self).__init__()
        self.fc1 = nn.Linear(dim, dim*expand_ratio)
        self.act1 = nn.GELU()
        self.dwconv = nn.Conv2d(dim*expand_ratio, dim*expand_ratio, kernel_size=(kernel_size, kernel_size), groups=dim*expand_ratio, padding=(kernel_size//2, kernel_size//2))
        self.act2 = nn.GELU()
    ...
    def forward(self, x, prev_msa=None):
        ...
        x = self.act2(self.dwconv(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1))
        x =  self.fc2(x)
        y = self.eca(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        return y

class EfficientBlock_wo_ECA(nn.Module):
    """Efficient Block to replace the Original Transformer Block"""
    def __init__(self, dim, expand_ratio = 2, kernel_size = 5):
        super(EfficientBlock_wo_ECA, self).__init__()
        self.fc1 = nn.Linear(dim, dim*expand_ratio)
        self.act1 = nn.GELU()
        self.dwconv = nn.Conv2d(dim*expand_ratio, dim*expand_ratio, kernel_size=(kernel_size, kernel_size), groups=dim*expand_ratio, padding=(kernel_size//2, kernel_size//2))
        self.act2 = nn.GELU()
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        assert router_name in ["CoordinateDescent", "Linear"]
        # if router_name == "CoordinateDescent":
        ...
        #     )
        # else:
        #     self.router = LinearRouting(in_channels = dim)
        ...
        if use_efficient_block:
            if skip_module == "Block":
                # self.efficient_block = EfficientBlock(dim = dim) # 忘记用act了
                self.efficient_block = EfficientBlockPro(dim = dim) # 忘记用act了
            elif skip_module == "MSA":
                self.efficient_block = EfficientBlock_MSA(dim = dim)
            elif skip_module == "MSA_PRO":
                self.efficient_block = EfficientBlock_MSA_PRO(dim = dim)
            elif skip_module == "ECA_wo_shortcut":
                self.efficient_block = EfficientBlock_ECA_wo_shortcut(dim = dim)
            elif skip_module == "ECA_wo_DWC":
                self.efficient_block = EfficientBlock_ECA_wo_DWC(dim = dim)
            elif skip_module == "wo_ECA":
                self.efficient_block = EfficientBlock_wo_ECA(dim=dim)
            elif skip_module == "ALR":
                self.efficient_block = EfficientBlock_ALR(model_dimension = dim)
            elif skip_module == "None":
                self.efficient_block = EfficientBlock_None(dim = dim)
    ...
    def msa_forward(self, x, policy, H, W):
        ...
        if self.window_size > 0:
            ...
            # if x.shape[-1] == 768:
            #     print("------divide win atten -----")
            #     number_windows = x.shape[0] // 2
            #     x[:number_windows, ...] = self.attn(x[:number_windows, ...], policy, H = H, W = W)
            #     x[number_windows:, ...] = self.attn(x[number_windows:, ...], policy, H = H, W = W)
            # else:
    ...
    def forward(self, x, policy=None):
        ...
        if self.use_efficient_block:
            if self.training:
                ...
                if torch.any(selected_indices == 1): # 不能if True in selected_indices:, fvcore测flops会报错, 也不能torch.any(selected_indices == True)
        ...
        # else:
        #     if x.shape[-1] == 768: # 424128
        #         print("===================chai===================")
        #         h,w = x.shape[1], x.shape[2]
        #         x = x.reshape(1, -1, 768)
        #         half_number_token = x.shape[1] // 2
        #         x[:, :half_number_token, :] = x[:, :half_number_token, :] + self.mlp_forward(x[:, :half_number_token, :])
        #         x[:, half_number_token:, :] = x[:, half_number_token:, :] + self.mlp_forward(x[:, half_number_token:, :])
        #         x = x.reshape(1, h, w, 768)
...
class ViT(Backbone):
    ...
    def __init__(
        ...
        for i in range(depth):
            block = Block(
                ...
                # use_efficient_block=False if (i == 0 and skip_all_block==False)  else True,
        ...
        for i in range(depth - len(window_block_indexes)):
            if router_module == "Ours":
                router = DynamicRouter(embed_dim = embed_dim)
            elif router_module == "Random":
                router = RandomRouter(embed_dim= embed_dim)
            elif router_module == "Small":
                router = SmallRouter(embed_dim=embed_dim)
            elif router_module == "Tiny":
                router = TinyRouter(embed_dim=embed_dim)
            elif router_module == "SE":
                router = SERouter(embed_dim=embed_dim)
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks): # x.shape = torch.Size([16, 32, 32, 384])
            if i in self.window_block_indexes or i == 0: # 第一个transformer block不进行router
                x = blk(x, policy = None)
                continue
            if self.router_module == "SE":
                pred_score = self.routers[count](x.permute(0, 3, 1, 2), prev_decision).reshape(B, -1, 2) # B, N, 2
            else:
                pred_score = self.routers[count](x.reshape(B, -1, C), prev_decision).reshape(B, -1, 2) # B, N, 2
            count = count + 1
            if self.training:
                if self.router_module == "Random":
                ...
                # if self.multi_score == True: 不能在这里乘score!!要用msa乘
                #     x = x * (hard_keep_decision + (1. - hard_keep_decision).detach()).reshape(B, H, W, -1)
            else:
                """gumbel_softmax"""
                ...
                hard_keep_decision = torch.zeros_like(pred_score[..., :1])
                hard_keep_decision[pred_score[..., 0] > pred_score[..., 1]] = 1.
                # max_token_number = 18500 if self.embed_dim == 384 else 17000 # vit_s & vit_s_1024
                # max_token_number = 17500 # vit_s_1024 for UHR
                if hard_keep_decision.sum().item() > self.max_number_token:
                    # print("=================================================================")
                    # print(f"This image OOM, decrease the number of tokens in Global attention: {self.max_number_token}")
                    # print("=================================================================")

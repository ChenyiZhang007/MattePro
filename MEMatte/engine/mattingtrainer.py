...
class MattingTrainer(AMPTrainer):
    ...
    def run_step(self):
        ...
        #     loss_dict[f'{i}_block_token_ratio'] = out_pred_prob[i].sum() / out_pred_prob[i].numel()
        # loss_dict['total_token_ratio'] = sum([p.sum() / p.numel() for p in out_pred_prob]) / len(out_pred_prob)
        if self.iter % 20 == 0:

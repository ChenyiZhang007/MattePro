...
class SAM2ImagePredictor:
    ...
    def predict_batch_boxes_and_features(
        ...
        box_labels[box_coords.sum((1,2))==-4] = torch.tensor([-1, -1], device=boxes.device, dtype=torch.int32)
        # we merge "boxes" and "points" into a single "concat_points" input (where
        # boxes are added at the beginning) to sam_prompt_encoder
        

        click_coords = clicks[:,:,:2]
        click_labels = clicks[:,:,2]

        concat_points = (torch.concat((box_coords,click_coords),dim=1), torch.concat((box_labels,click_labels),dim=1))

...
class MattePro(nn.Module):
    ...
    def forward(self, batched_inputs):  # image: [1, 3, 643, 960]: 0.0~1.0, trimap: [1, 1, 643, 960]: 0.0~1.0
        ...
        # else:
        #     condition_proj = None
        image = batched_inputs[0].to(self.device)
        click = batched_inputs[1][0].to(self.device)
        bbox = batched_inputs[1][1].to(self.device)

        # print('bbox',bbox.device)
        # print('model',self.device)

        assert bbox.device==self.device
        assert image.device==self.device
        

        interm_features, sam2_logits, matting_logits, pred_trimap = self.forward_samhq(image,click,bbox)
        return F.softmax(matting_logits, dim=1)
        ...
        # else:
            # return matting_logits
        
    def forward_image_encoder(self, images, condition_proj=None):
    ...
    # @torch.no_grad()
    def forward_others(self, images, prompt, features, interm_features):
        
        sam2_logits, trimap_logits = self.sam_predictor.predict_batch_boxes_and_features(prompt, features)
        return features, sam2_logits, trimap_logits
        
        

    def forward_samhq(self, images, click, bbox):
        if self.lora_rank is None:
            with torch.no_grad():
                features, interm_features, pred_trimap = self.forward_image_encoder(images, (click, bbox))
        else:
            features, interm_features, pred_trimap = self.forward_image_encoder(images, (click, bbox))

        return self.forward_others(images, (click, bbox), features, interm_features) + (pred_trimap, )
